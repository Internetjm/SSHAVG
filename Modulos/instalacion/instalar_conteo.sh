...
SERVICIO="/etc/systemd/system/jmvpn-conteo.service"
NVM_DIR="/root/.nvm"
...
const TOKEN = 'jmvpn2024secret';  // Mismo que en ContadorOnlineVPS.java
...
systemctl enable jmvpn-conteo 2>/dev/null || true
systemctl restart jmvpn-conteo 2>/dev/null || service jmvpn-conteo restart 2>/dev/null || true
sleep 3

if systemctl is-active --quiet jmvpn-conteo 2>/dev/null; then
    ok "Servicio jmvpn-conteo activo"
else
    aviso "Error con systemd. Revisando logs..."
    journalctl -u jmvpn-conteo -n 15 --no-pager 2>/dev/null || true
...
CT=$(curl -s -H "X-Token: jmvpn2024secret" "http://127.0.0.1:8081/ct" 2>/dev/null)
STATUS=$(curl -s -H "X-Token: jmvpn2024secret" "http://127.0.0.1:8081/status" 2>/dev/null)

if echo "$STATUS" | grep -q "uptime"; then
    ok "Servidor HTTP respondiendo correctamente"
else
    aviso "Servidor no responde. Revisa: journalctl -u jmvpn-conteo -f"
fi
...
echo -e "║  Servicio:  jmvpn-conteo (systemd)"
echo -e "║  HTTP:      http://$IP:8081"
echo -e "║  TCP:       $IP:8082 (fallback)"
echo -e "║  Token:     jmvpn2024secret"
echo -e "║"
...
echo -e "║  → Logs en vivo:  journalctl -u jmvpn-conteo -f"
echo -e "║  → Ver conteos:   curl -H 'X-Token: jmvpn2024secret' \\"
echo -e "║                        http://127.0.0.1:8081/ct"
echo -e "║  → Ver usuarios:  curl -H 'X-Token: jmvpn2024secret' \\"
echo -e "║                        http://127.0.0.1:8081/debug"
echo -e "║  → Reiniciar:     systemctl restart jmvpn-conteo"
echo -e "╚══════════════════════════════════════════════════════════════╝${NC}"
